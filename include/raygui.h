...
RAYGUIAPI int GuiRadioButton(Rectangle bounds, const char *text, bool *checked);
...
static void GuiDrawCircle(Rectangle rec, int borderWidth, Color borderColor, Color color);
...
// Check Box control, returns true when active
int GuiRadioButton(Rectangle bounds, const char *text, bool *checked)
{
    int result = 0;
    GuiState state = guiState;
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiSliderDragging)
    {
        ...
        if (CheckCollisionPointRec(mousePoint, totalBounds))
        {
            ...
            if (IsMouseButtonReleased(MOUSE_LEFT_BUTTON)) *checked = true;
        }
    }
    //--------------------------------------------------------------------
    ...
    //--------------------------------------------------------------------
    //GuiDrawRectangle(bounds, GuiGetStyle(CHECKBOX, BORDER_WIDTH), GetColor(GuiGetStyle(CHECKBOX, BORDER + (state*3))), BLANK);
    GuiDrawCircle(bounds, GuiGetStyle(CHECKBOX, BORDER_WIDTH), GetColor(GuiGetStyle(CHECKBOX, BORDER + (state*3))), BLANK);
    if (*checked)
    {
        Rectangle check = { bounds.x + GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING),
                            bounds.y + GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING),
                            bounds.width - 2*(GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING)),
                            bounds.height - 2*(GuiGetStyle(CHECKBOX, BORDER_WIDTH) + GuiGetStyle(CHECKBOX, CHECK_PADDING)) };
        GuiDrawCircle(check, 0, BLANK, GetColor(GuiGetStyle(CHECKBOX, TEXT + state*3)));
    }
...
static void GuiDrawCircle(Rectangle rec, int borderWidth, Color borderColor, Color color)
{
    ...
    {
        // Draw rectangle filled with color
        DrawCircle((int)(rec.x + rec.width/2), (int)(rec.y + rec.height/2), (int)rec.width/2, GuiFade(color, guiAlpha));
    }

    if (borderWidth > 0)
    {
        DrawCircleLines(rec.x + rec.width/2, (int)(rec.y + rec.height/2), rec.width/2, GuiFade(borderColor, guiAlpha));
        DrawCircleLines(rec.x + rec.width/2, (int)(rec.y + rec.height/2), rec.width/2 + 0.25, GuiFade(borderColor, guiAlpha));
        DrawCircleLines(rec.x + rec.width/2, (int)(rec.y + rec.height/2), rec.width/2 + 0.5, GuiFade(borderColor, guiAlpha));
        DrawCircleLines(rec.x + rec.width/2, (int)(rec.y + rec.height/2), rec.width/2 + 0.75, GuiFade(borderColor, guiAlpha));
        DrawCircleLines(rec.x + rec.width/2, (int)(rec.y + rec.height/2), rec.width/2 + 1, GuiFade(borderColor, guiAlpha));
    }
}
